...
public class StationManager implements Iterable<Station> {
    ...
    private SphericalGeometry sphericalGeometry;
    ...
    public Station findNearestTo(LatLon pt) {
        //SphericalGeometry sphericalGeometry=new SphericalGeometry();
        double distance=10000;
        Station stn=null;
        List<Station> l=new ArrayList<>();
        for (Station s:stns){
            if (SphericalGeometry.distanceBetween(s.getLocn(), pt)<distance){
                distance=SphericalGeometry.distanceBetween(s.getLocn(), pt);
                stn=s;
            }
        }
        return stn;
